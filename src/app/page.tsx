    ...
    <main className="min-h-screen bg-gradient-to-b from-amber-50 via-orange-50 to-white text-gray-900">
      ...
      <section className="max-w-6xl mx-auto px-4 py-12 sm:py-20">
        <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr] items-center">
          <div className="space-y-6">
            ...
            <p className="text-lg sm:text-xl text-amber-800 max-w-xl">
            ...
            <div className="flex flex-wrap gap-6 text-sm text-amber-700">
              <div>👥 1200+ gäster</div>
              <div>🏡 75+ kvarter</div>
              <div>⭐ 4,9/5 i omdöme</div>
            </div>
          </div>

          <div className="bg-white/80 rounded-3xl shadow-lg p-8 border border-amber-100" data-reveal>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            <div key={feature.title} className="bg-white rounded-2xl shadow-lg p-6 border border-amber-100">
              ...
              <p className="text-amber-700 text-sm">{feature.text}</p>
            </div>
          ))}
        </div>
      </section>

      <section className="max-w-6xl mx-auto px-4 py-12" data-reveal>
        <div className="bg-white rounded-3xl shadow-lg p-8 border border-amber-100">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6 mb-8">
            <div>
              <h2 className="text-3xl font-bold text-amber-900">Så funkar det</h2>
              <p className="text-amber-700 mt-2">Välj ditt perspektiv och se hur kvällen flyter.</p>
            </div>
            <div className="flex gap-2 bg-amber-50 rounded-full p-1">
              <button
          ...
          <div className="grid gap-4 md:grid-cols-4">
            {steps.map((step, index) => (
              <div key={step.title} className="rounded-2xl border border-amber-100 bg-amber-50/60 p-5">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-2xl">{step.icon}</span>
                  <span className="text-sm font-semibold text-amber-700">0{index + 1}</span>
                </div>
                <h3 className="font-semibold text-amber-900 mb-2">{step.title}</h3>
      ...
      <section className="max-w-6xl mx-auto px-4 pb-12" data-reveal>
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr] items-center">
          <div className="space-y-4">
            <h2 className="text-3xl font-bold text-amber-900">Perfekt för föreningar, kvarter och vängrupper</h2>
            <p className="text-amber-700">
              Cykelfesten är byggd för att skapa gemenskap där du bor. Från Piteå till hela Sverige — enkelt, tryggt och inspirerande.
            </p>
          ...
          <div className="bg-white rounded-3xl shadow-lg p-6 border border-amber-100">
            <h3 className="font-semibold text-amber-900 mb-3">Vad arrangörer säger</h3>
            <p className="text-amber-700 text-sm">
              “Vi fick 36 grannar att mötas på en kväll, utan att behöva Excel eller utskrifter.”
            </p>
            <p className="text-amber-600 text-xs mt-4">— Testimonial kommer snart</p>
          </div>
        </div>
      </section>

      <section className="max-w-6xl mx-auto px-4 pb-16" data-reveal>
        <div className="bg-gradient-to-r from-amber-500 to-orange-500 rounded-3xl p-8 text-white shadow-lg">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
            <div>
              <h2 className="text-3xl font-bold">Redo att starta nästa Cykelfest?</h2>
              <p className="text-white/90 mt-2">Skapa eventet på fem minuter — vi tar hand om resten.</p>
            </div>
            <div className="flex flex-col sm:flex-row gap-3">
              <Link
                href="/organizer/login"
                className="rounded-full bg-white text-amber-700 px-6 py-3 text-sm font-semibold text-center"
              >
                Skapa ett event
              </Link>
              <Link
