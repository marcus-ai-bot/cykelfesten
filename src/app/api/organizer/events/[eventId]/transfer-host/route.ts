...
export async function POST(
  ...
  for (const course of courses) {
    const cascade = await cascadeChanges({
      ...
      matchPlanId,
      type: 'transfer_host',
      coupleId: from_couple_id,
      details: {
        toCoupleId: to_couple_id,
        courses: [course],
      },
    });

    if (!cascade.success) {
      results.push(`❌ ${course}: ${cascade.errors.join(', ')}`);
      continue;
    }

    if (cascade.assignmentsCreated === 0 && cascade.pairingsCreated === 0 && cascade.envelopesUpdated === 0) {
      results.push(`⚠️ ${course}: Ingen värd-assignment att överföra`);
      continue;
    }

    results.push(
      `✅ ${course}: Värdskap överfört (${cascade.pairingsCreated} pairings, ${cascade.envelopesUpdated} kuvert)`
    );
  }
