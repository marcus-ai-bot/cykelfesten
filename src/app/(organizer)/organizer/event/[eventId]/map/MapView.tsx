...
export function MapView({ eventId, eventName }: { eventId: string; eventName: string }) {
  ...
  const activeCouples = useMemo(() => {
    return data.couples.filter((c) => c.isConfirmed && !c.isCancelled);
  }, [data.couples]);

  const sortedCouples = useMemo(() => {
    return [...activeCouples].sort((a, b) => a.name.localeCompare(b.name));
  }, [activeCouples]);
  ...
  const totalCouples = activeCouples.length;
  ...
  return (
    <div className="h-screen bg-gray-50 flex flex-col">
      {/* Header */}
      <MapHeader eventId={eventId} eventName={eventName} coupleCount={activeCouples.length} missingCount={data.missingCoords.length} />
      ...
      <div className="relative flex-1 min-h-0">
        ...
        {!activeCourse && !selectedGroup && activeCouples.length > 0 && (
