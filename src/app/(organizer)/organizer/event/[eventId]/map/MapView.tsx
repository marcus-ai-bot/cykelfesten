...
export function MapView({ eventId, eventName }: { eventId: string; eventName: string }) {
  ...
  useEffect(() => {
    ...
    if (selectedGroup && activeCourse) {
      ...
      map.setPaintProperty(`host-${activeCourse}-fill`, 'circle-opacity',
        ['case', ['==', ['get', 'hostId'], selectedGroup.hostId], 1, 0]);
  ...
  const inactiveCount = data.couples.filter(c => c.isCancelled || !c.isConfirmed).length;
        ...
        <div className={`hidden md:block absolute left-0 top-0 z-10 w-80 transition-all duration-300 ${selectedGroup ? 'opacity-0 pointer-events-none -translate-x-4' : 'opacity-100 translate-x-0'}`}>
          <div className="bg-white/95 backdrop-blur shadow-lg rounded-r-xl border border-gray-200/80 max-h-[calc(100vh-8.5rem)] overflow-y-auto">
            <div className="p-4 pb-3 space-y-3 border-b border-gray-100">
              ...
              />
              <div className="text-xs text-gray-500">{countLabel}{inactiveCount > 0 ? ` · ${inactiveCount} inaktiva` : ''}</div>
            </div>
            ...
            <div ref={sidebarListRef} className="py-1">
              {filteredCouples.map((couple) => {
                ...
                const isInactive = couple.isCancelled || !couple.isConfirmed;
                return (
                  <div key={couple.id} ref={setSidebarRowRef(couple.id)} className="px-1">
                    <button
                      type="button"
                      onClick={() => handlePinClick(couple.id)}
                      className={`w-full text-left px-3 py-2.5 border-l-4 rounded-r-lg transition ${isSelected ? 'bg-orange-50' : 'bg-transparent'} hover:bg-gray-50 border-transparent ${isInactive ? 'opacity-50' : ''}`}
                      style={isSelected && cfg ? { borderColor: cfg.color } : undefined}
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0">
                          <div className={`text-sm font-medium truncate ${isInactive ? 'text-gray-400 line-through' : 'text-gray-900'}`}>{couple.name}</div>
                          <div className="text-xs text-gray-500 truncate">{couple.address}</div>
                        </div>
                        {isInactive ? (
                          <span className="text-xs whitespace-nowrap px-2 py-0.5 rounded-full text-gray-500 bg-gray-100">
                            {couple.isCancelled ? 'Avbokad' : 'Ej bekräftad'}
                          </span>
                        ) : role ? (
                          <span
